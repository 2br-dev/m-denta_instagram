...
//		$con = new mysqli("localhost", "mdenta_local", "38180067", "burdilo_mdenta");
//
...
//		}
        ...
        $subject = 'Новое сообщение, на сайте ' . $domen . '';
		...
		if ($phone == '')
		{
			$response['error']['phone'] = 1;
			$status = false;
		}

		if ($name == '')
		{
            $response['error']['name'] = 1;
			$status = false;
		}
        ...
        if ($status) {
            O('_mdd_appointment')->create(array(
                            's:name'        =>  $name,
                            's:phone'       =>  $phone,
                            's:comment'     => $comment,
                            'e:created'     =>  'NOW()',
            ...
            $body .= 'Телефон: <a href="tel:'. $phone .'"><strong>'. $phone .'</strong></a><br>';
            $body .= 'Имя: <strong>'. $name .'</strong><br>';
            if($comment){
                $body .= 'Комментарий: <strong>'. $comment .'</strong>';
            }
            $emails = getfl('emails');
...
//                ->setPassword('1878manu');
                ->setUsername('com23-robot@mail.ru')
                ->setPassword('369Robot+741');
            ...
            $message = (new \Swift_Message($subject))
                ->setFrom(['com23-robot@mail.ru' => 'Заявка с сайта'])
                ->setTo($emails)
